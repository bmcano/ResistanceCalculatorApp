...
class ResistanceSmdFormatterTest {
    ...
    fun `3 digit conversions`() {
        ...
        resistor.code = "120"
        assertEquals("12.0 ${Symbols.OHMS}", ResistanceSmdFormatter.execute(resistor))
        resistor.code = "1R2"
        ...
        resistor.code = "123"
        resistor.units = ""
        assertEquals("12000.0 ${Symbols.OHMS}", ResistanceSmdFormatter.execute(resistor))
    }
    ...
    fun `4 digit conversions`() {
        ...
        assertEquals("Enter code", ResistanceSmdFormatter.execute(resistor))
        resistor.code = "1231"
        assertEquals("1230.0 ${Symbols.OHMS}", ResistanceSmdFormatter.execute(resistor))
        resistor.code = "1R22"
    ...
    fun `EIA96 conversions`() {
        ...
        assertEquals("Enter code", ResistanceSmdFormatter.execute(resistor))
        resistor.code = "12A"
        assertEquals("130.0 ${Symbols.OHMS}", ResistanceSmdFormatter.execute(resistor))
        resistor.code = "1XM"
        assertEquals("${Double.NaN}", ResistanceSmdFormatter.execute(resistor))
    }
}